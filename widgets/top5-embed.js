...
(function () {
  ...
  ready(function(){
    ...
    const debug = (S.getAttribute('data-debug') === '1');
    const log = (...a)=> debug && console.log('[top5-embed]', ...a);
    const error = (...a)=> debug && console.error('[top5-embed]', ...a);
    ...
    fetch(jsonUrl, {cache:'no-store'}).then(r=>{
      if (!r.ok) throw new Error('HTTP '+r.status);
      return r.json();
    }).then(data=>{
      if (singleTargetSel) {
        const el = document.querySelector(singleTargetSel);
        if (!el){ error('target not found:', singleTargetSel); return; }
        renderInto(el, singleSection, data);
      } else {
        const nodes = Array.from(document.querySelectorAll(multiSelector));
        if (!nodes.length){ error('no nodes for selector', multiSelector); return; }
        nodes.forEach(el=>{
      ...
      if (singleTargetSel){
        ...
        document.querySelectorAll(multiSelector).forEach(el=>renderError(el,'Erreur de chargement'));
      }
    });
  });
})();
