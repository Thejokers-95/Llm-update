// Embeds Top 5 without iframe, no deps.
// Usage:
// <div id="llm-top5-code"></div>
// <script src="https://cdn.jsdelivr.net/gh/Thejokers-95/Llm-update@main/widgets/top5-embed.js"
//         data-target="#llm-top5-code" data-section="code"></script>

(function(){
  const S = document.currentScript;
  const targetSel = S.getAttribute('data-target') || '#llm-top5';
  const section = (S.getAttribute('data-section') || 'code').toLowerCase();
  const jsonUrl = S.getAttribute('data-json') ||
    S.src.replace(/\/widgets\/top5-embed\.js.*$/, '/top-leaderboards.json');

  const el = document.querySelector(targetSel);
  if (!el){ console.error('top5-embed: target not found'); return; }
  ...
  fetch(jsonUrl, {cache:'no-store'})
    .then(r=>r.json())
    .then(d=>render(d[section]||[]))
    .catch(e=>{ el.textContent='Erreur de chargement'; console.error(e); });
})();
